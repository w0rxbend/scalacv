...
- **Batteries included.** Filters and morphology, geometric transforms, drawing, contours and Hough, colour segmentation; Haar/YuNet faces, QR, ArUco, ONNX inference; a high-level `Camera`/`Recorder`; **motion detection** for a static/MJPEG cam; **pose estimation** (skeletons, hands, head pose); **gesture recognition**; **video-conferencing** background blur & virtual backgrounds; and **screen analysis** (template matching, change detection).
